...
void main()
{
    ...
    vec4 shadeColor= texture2D(u_tex0, uv); //取Kamakurashi
    float shading= shadeColor.r*0.299+shadeColor.g*0.587+shadeColor.b*0.144;            //取Kamakurashi綠色版作為明亮值
    ...
    vec4 c;
                ...
                if( shading > 5. * step ){
                    c = mix( texture2D( u_tex1, vUv ), vec4( 1.0,0.349,0.0,1.0 ), 6. * ( shading - 5. * step ) );
                }
                
     vec4 inkColor = vec4(0.0, 0.4667, 1.0, 1.0);
     vec4 src = mix( mix( inkColor, vec4( 1.0,0.349,0.0,1.0), c.r ), c, .5 );
